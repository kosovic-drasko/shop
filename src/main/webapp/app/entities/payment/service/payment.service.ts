...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'createTime' | 'updateTime'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      createTime: payment.createTime?.toJSON() ?? null,
      updateTime: payment.updateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      createTime: restPayment.createTime ? dayjs(restPayment.createTime) : undefined,
      updateTime: restPayment.updateTime ? dayjs(restPayment.updateTime) : undefined,
    };
  }
