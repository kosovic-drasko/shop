...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'createTime' | 'updateTime'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      createTime: product.createTime?.toJSON() ?? null,
      updateTime: product.updateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      createTime: restProduct.createTime ? dayjs(restProduct.createTime) : undefined,
      updateTime: restProduct.updateTime ? dayjs(restProduct.updateTime) : undefined,
    };
  }
