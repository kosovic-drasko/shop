...
export class ProductInOrderMainRoutingResolveService implements Resolve<IProductInOrderMain | null> {
  constructor(protected service: ProductInOrderMainService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IProductInOrderMain | null | never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((productInOrderMain: HttpResponse<IProductInOrderMain>) => {
