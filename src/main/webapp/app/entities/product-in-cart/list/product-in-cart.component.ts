...
import { EntityArrayResponseType, ProductInCartService } from '../service/product-in-cart.service';
...
export class ProductInCartComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProductInCart): number => this.productInCartService.getProductInCartIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.productInCarts = this.refineData(dataFromBody);
  }

  protected refineData(data: IProductInCart[]): IProductInCart[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IProductInCart[] | null): IProductInCart[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.productInCartService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
